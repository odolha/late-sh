...
- Last updated: 2026-07-11 (Embedded webview split out of the `late` binary on Linux: the webview code moved to the new `late-webview` crate, shipped as a standalone helper binary next to `late`, so `late` no longer links WebKitGTK/GTK and starts fine on hosts without the webview libraries — installing the distro webkit/gstreamer packages later enables embedded YouTube without reinstalling. Windows/macOS keep the in-process `late webview-pair` path via the same crate as a library. See §17. Same day: Radio is now the default audio source: `AudioSource::default()` = `Radio`, so users with no persisted `audio_source` land on Nightride FM (default station still Chillsynth) instead of Icecast; unknown persisted values also fall back to `radio`. The sidebar music stage dock reordered to radio → youtube → icecast, and `v+x` cycles in that dock order.)
...
- `set_playback_source: youtube` spawns a helper child (token on stdin) only when `embedded_webview_enabled=true`. On Linux the helper is the standalone `late-webview` binary (resolved via `LATE_WEBVIEW_BIN`, then a sibling of the `late` executable, then `$PATH`); on Windows/macOS it is `late webview-pair`, the same binary re-executed with the webview compiled in.
