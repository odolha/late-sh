...
- **Feed events (no reward, no persistence).** The same screen scrape also posts visible activity-feed events (`ActivityKind::GameEvent`, category `Game`) for non-reward moments: launching ("started a NetHack game", on `connect`), reaching a new deepest dungeon level (parsed from the `Dlvl:` status field, but **throttled to every 5th level** via `DESCENT_EVENT_STEP` — the Amulet is ~25-30 levels down, so a per-level event buried the feed; the event fires only when the deepest level crosses into a deeper 5-level band, keyed off the band not `dlvl % 5` so a trapdoor chain still posts once), and dying (with the dungeon level died on, from the last `Dlvl:` seen before the tombstone hides the status line). Ascension is a *win* (`GameWon`), not one of these events, and has no dungeon level — it happens on the Astral Plane. These are flavor only — no chips, no badges, nothing written to the DB beyond the in-memory activity stream. Depth uses a state comparison (deepest-seen) rather than counting events; death matching keys off end-of-game-only signals (the death disclosure prompt and the "REST IN PEACE" tombstone), deliberately *not* the message-line "You die...", which NetHack prints before the life-saving check (an amulet-of-life-saving survivor would otherwise post a spurious death). Quit/save/ascension are excluded.
...
| `state.rs` | Per-session `State`: launcher/running `Mode`, connection config (host/port/secret/term/enabled), the optional `NethackProcess`, last viewport `Rect`, the post-exit input grace, input interception/forwarding (`intercept_input` remaps F1→`?`, `forward_input`, `strip_input_noise`), and the screen scrape in `tick` (`scan_screen`: milestones + descent + death) with per-session debounce flags (incl. `deepest_dlvl`, `death_noted`) + the optional `NethackAwards` sink. Emits the "started" feed event on `connect`. |
| `milestone.rs` | Pure screen-text matchers for the Amulet pickup, ascension, and death messages (verified against NetHack 5.0.0 source). Reward markers (Amulet/ascension) are matched only at the **start of the top message line** (row 0) to defeat the easy spoofs (engravings are prefixed, named text is embedded, map/menu text is off row 0); ascension also needs the choir prelude to have led row 0 earlier. Not bulletproof — a player crafting a pline that *begins* with a marker could still spoof; accepted as low-stakes flair. Death matches end-of-game disclosure/tombstone only (full screen). |
| `status.rs` | Pure parser for the `Dlvl:` status field (`parse_dlvl`). Read as a value; `state.rs` tracks the deepest seen and posts a "descended" event on a new max (a state comparison, so no repeated-event-counting problem). |
| `award.rs` | `NethackAwards`: the chip-payout + profile-badge grant sink **and** the feed-event poster. Milestone grants are fire-and-forget, once-per-account (lifetime reward template `nethack_amulet`/`nethack_ascension` + `grant_unique_milestone_award`); ascension back-grants the Amulet award. `note_event` posts non-reward NetHack moments (start/descend/death) to the activity feed (`ActivityKind::GameEvent`, category `Game`, visible). Holds `ChipService`/`Db`/`ActivityPublisher`. |
...
- Milestone reward scraping is best-effort, not authoritative. Markers are anchored to the start of row 0 (the message line) in `milestone.rs` to defeat casual spoofing (engrave/name the marker then look), and the once-per-account guarantee comes from the lifetime reward template + `NOT EXISTS` award insert in `award.rs` (plus per-session debounce in `state.rs`). Residual risk is accepted (cosmetic flair, not a competitive economy): a player who engineers a pline that *begins* with a marker can still spoof. The only spoof-proof source is NetHack's host-side xlog/logfile, which would need a cross-crate signal — deliberately not built. Match on the exact NetHack 5.0.0 strings (re-verify on any version bump).
...
- Client `milestone.rs`: real-Amulet pickup matches but the (fake-shared) inventory line does not; ascension line matches both genders; choir prelude matches; markers must *lead* row 0 (engraving/named/off-row spoofs rejected); death matches the end-of-game disclosure/tombstone but NOT the pre-life-saving "You die..." line, nor quit/save/ascend. Client `status.rs`: `parse_dlvl` reads standard/padded/shrunk forms and returns `None` for non-numeric branches. The `state.rs` award sink is `None` in unit tests, so the scrape logic runs without a DB.
