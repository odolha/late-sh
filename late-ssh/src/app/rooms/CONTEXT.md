...
- `chess/settings.rs` offers two clock modes for new tables (`blitz`, `rapid`); `daily` is no longer offered because correspondence chess moved to the daily-games domain (`late-ssh/src/app/daily/CONTEXT.md`), but `ChessTimeControl::Daily` and its `from_id` parsing are kept so existing daily table rows deserialize until they finish or idle out. Missing/unknown persisted values fall back to the default rapid control.
...
- `main.rs` starts a process-wide recent-room-join feed from all room-game event streams, keeps a bounded in-memory history, and gives each `App` a receiver plus an initial history snapshot for the Home multiplayer box. The history is process-local and best-effort like the activity feed: broadcast lag is logged but not replayed. The same registry task is the single choke point that publishes `ActivityEvent::sat_down` per seat join (exhaustive `GameKind` → `ActivityGame` map in `registry.rs`); the lounge feed (`app/activity/lounge.rs`) turns those into `#lounge` system lines, with a per-(user, game) 10-minute repeat window absorbing leave/re-sit churn.
...
- There are two seats: White and Black. Entering starts as a viewer; `s`, `Space`, or `Enter` sits in the first open color. `n` starts a game when both seats are occupied and both players have readied; legacy daily tables use the same seating/readiness flow as blitz and rapid.
...
- Time controls are preset-only and intentionally generous: blitz is `5+3` and rapid is `15+10`. Room settings store `blitz`, `rapid`, or the legacy `daily` (`1d/move`, no longer creatable); old seven-preset IDs fall back to rapid. Countdown clocks debit elapsed time idempotently as clock state is settled and add increment after a legal move. Legacy daily tables use a per-move deadline instead of a banked player clock.
