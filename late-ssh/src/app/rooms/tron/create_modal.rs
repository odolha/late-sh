...
impl TronCreateModal {
    ...
    fn submit(&mut self) -> CreateModalAction {
        ...
        if display_name.is_empty() {
            self.error = Some("Grid name is required.".to_string());
...
impl CreateRoomModal for TronCreateModal {
    fn draw(&self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_widget(Paragraph::new(section_heading("Grid")), layout[1]);
        frame.render_widget(
