...
struct Shared {
    bin: String,
    /// The one shared high-score file every child gets as `-f`.
    score_file: String,
...
pub struct ClientHandler {
    ...
    cols: u16,
    rows: u16,
    /// The running dopewars child, once the shell is requested.
    host: Option<PtyHost>,
}
...
/// The TERM to hand the dopewars child: the client's own if the host can resolve
/// its terminfo, otherwise a universal fallback. dopewars (ncursesw) aborts on an
/// unrecognized TERM, but every modern terminal renders `xterm-256color`, so this
/// keeps clients on exotic terminals (kitty/ghostty/wezterm, which ship their own
/// terminfo) playable.
...
impl Handler for ClientHandler {
    ...
    async fn auth_publickey(&mut self, user: &str, key: &PublicKey) -> Result<Auth, Self::Error> {
        ...
        tracing::info!(user, "client authorized");
    ...
    async fn shell_request(
        ...
        // dopewars' ncursesw aborts on a TERM it has no terminfo for; fall back to
        ...
        self.host = Some(PtyHost::spawn(
            HostConfig {
                ...
                score_file: self.shared.score_file.clone(),
                cols: self.cols,
                rows: self.rows,
                term,
            },
...
mod tests {
    ...
    fn unknown_term_falls_back_to_xterm_256color() {
        ...
        // must fall back so dopewars doesn't abort with "Unknown terminal type".
        assert_eq!(
